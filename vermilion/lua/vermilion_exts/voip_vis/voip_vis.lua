...
EXTENSION.Name = "VoIP Visualiser"
EXTENSION.ID = "voip_vis"
EXTENSION.Description = "Visualises the VoIP for a player"
...
function EXTENSION:InitClient()
	...
	self:AddHook(Vermilion.EVENT_EXT_LOADED, function()
		if(Vermilion:GetExtension("dermainterface") != nil) then
			Vermilion:AddClientOption("Render VoIP graphs", "vermilion_render_voip")
		end
	end)
	...
	self:AddHook("PostDrawOpaqueRenderables", function()
		...
		for i,k in pairs(player.GetAll()) do
			if(k.Vermilion_VoIPHistory != nil) then
